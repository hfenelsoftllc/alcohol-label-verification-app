...
CONTROLS = [
    ...
    ("RA-2", "Security Categorization", "Risk Assessment", "Implemented", "-",
     ...
     "baseline."),
    ("RA-3", "Risk Assessment", "Risk Assessment", "Implemented", "ISSUE 4.6 (#48)",
     "THREAT-MODEL.md applies STRIDE across all six required categories against the trust "
     "boundaries in DATA-FLOW-final.md, rating likelihood/impact/residual risk for 18 threats "
     "and calling out the Top 3 with owners and mitigation timelines. Reviewed and signed off "
     "by the project lead; RA-3 closed in POAM.md."),
    ...
    ("CA-5", "Plan of Action and Milestones", "Security Assessment and Authorization", "Implemented", "ISSUE 4.5, 4.6",
     "POAM.md tracks no remaining NIST control gaps (RA-3 closed by THREAT-MODEL.md, ISSUE "
     "4.6); it retains one forward-looking scalability item (T-D2, batch-concurrency cap) for "
...
def build_workbook() -> Workbook:
    ...
    summary_ws.append([])
    summary_ws.append(["Open items", "None -- all 33 controls Implemented. POAM.md retains one forward-looking "
                                       "scalability item (T-D2, batch-concurrency cap, pilot phase), not a control gap."])
